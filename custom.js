...
var variations = jQuery('.variations tr')
var variationsHtml = '';
for (var i = 0; i < variations.length; i++) {
	var title = jQuery(variations[i]).find('label').text()
	var option = jQuery(variations[i]).find('option')
	var selectId = jQuery(variations[i]).find('select').attr('id')

	optionTag = ''
	for (var j = 1; j < option.length; j++) {
		value = jQuery(option[j]).attr('value')
		if(selectId == 'color') {
			if(value == jQuery('#'+selectId).val()) {
				optionTag += `<li onclick="changeItem(this)" class="variant-item active" name="${selectId}" data-value="${value}" style="background-color: ${getColor(value)}"></li>`
			} else {
				optionTag += `<li class="variant-item" name="${selectId}" onclick="changeItem(this)" data-value="${value}" style="background-color: ${getColor(value)}"></li>`
			}
		} else {
			if(value == jQuery('#'+selectId).val()) {
				optionTag += `<li onclick="changeItem(this)" class="variant-item active" name="${selectId}" data-value="${value}">${value}</li>`
			} else {
				optionTag += `<li class="variant-item" name="${selectId}" onclick="changeItem(this)" data-value="${value}">${value}</li>`
			}
		}
	}
	...
	jQuery('#'+selectId).change(function() {
		...
		jQuery('[name='+id+']').removeClass('active')
		activeTag = jQuery('[name='+id+']')
		for (var k = 0; k < activeTag.length; k++) {
			if(jQuery(activeTag[k]).attr('data-value') == v) {
				jQuery(activeTag[k]).addClass('active')
			}
		}
	})

	variationsHtml += `<label for="${title.toLowerCase()}" style="font-size: 12pt">${title}</label>
	<ul class="variant-custom">
		${optionTag}
	</ul>`
}
jQuery('.variations').after(variationsHtml)
...
function changeItem(that) {
	...
	name = jQuery(that).attr('name')
	v = jQuery(that).attr('data-value')
	jQuery('[name='+name+']').removeClass('active')
